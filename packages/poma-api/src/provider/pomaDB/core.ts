...
/**
 * Returns an appropriate RPC endpoint retrieved from a pomaDB endpoint.
 * @param url - URL of a pomaDB service.
...
export async function getRPCEndpoint(url: string): Promise<string> {
  ...
  const data = response.data.nodes as PomaDbNode[];
  let nodes = data
    .filter(d => d.status)
...
export async function getBalance(
  ...
  const data = response.data as PomaDbBalance;
  ...
  log.info(`Retrieved Balance for ${address} from PomaDB ${url}`);
  return bal;
}
...
export async function getClaims(
  ...
  const data = response.data as PomaDbClaims;
  ...
  log.info(`Retrieved Claims for ${address} from PomaDB ${url}`);
...
export async function getMaxClaimAmount(
  ...
  const data = response.data as PomaDbClaims;
  log.info(
    `Retrieved maximum amount of gas claimable after spending all POMA for ${address} from PomaDB ${url}`
  );
...
export async function getTransactionHistory(
  ...
  const data = response.data as PomaDbHistory;
  log.info(`Retrieved History for ${address} from PomaDB ${url}`);
  return data.history.map(rawTx => {
    return {
      change: {
        NEO: new u.Fixed8(rawTx.POMA || 0),
