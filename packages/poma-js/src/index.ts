import apiPlugin from "@marcoslobo/poma-api";
import * as pomaCore from "@marcoslobo/poma-core";
import nepPlugin from "@marcoslobo/poma-nep5";

const pomaWithApi = apiPlugin(pomaCore);
const pomaJs = nepPlugin(pomaWithApi);
...
const create = {
  ...
  wallet: (k: pomaCore.wallet.WalletJSON) => new wallet.Wallet(k),
  ...
  query: (req: pomaCore.rpc.RPCRequest) => new rpc.Query(req),
  network: (net: Partial<pomaCore.rpc.NetworkJSON>) => new rpc.Network(net),
...
export default {
  ...
  add: {
    network: (network: pomaCore.rpc.Network, override = false) => {
